...
export class GoldLotteryComponent implements OnInit {
  ...
  public changeForm(typeForm): void {
    this.type = typeForm;
    this.formData = {
      code: '',
  ...
  private format(input, format, sep): any {
    let output = '';
    let idx = 0;
  ...
  public formValidate(addressType: string): void {
    switch (addressType) {
      case 'DUC':
        ...
        if (this.formData.addressDuc.length === 34 && ['L', 'l', 'M', 'm'].includes(this.formData.addressDuc.substring(0, 1))) {
          ...
          this.goldlotteryservice
            .getValidateDucatusAddress(this.formData.addressDuc)
            .then((result) => {
  ...
  private checkRegistrateForm(): void {
  ...
  public confirmRegistration(): void {
    ...
    this.goldlotteryservice
      .codeRegistrate(this.formData.addressDuc, this.formData.addressDucx, new UpperCasePipe().transform(this.formData.code))
      .then((result) => {
        if (result.token_id) {
  ...
  public wordsReturn(words, count): any {
    let word: string;
    const n = words.split(' ');
  ...
  public confirmCheck(): void {
    // const publicCode = this.formDataCheck.precode + '-' + this.formDataCheck.code;

    this.goldlotteryservice
      .codeCheck(new UpperCasePipe().transform(this.formDataCheck.code))
      .then((result) => {
        if (result.token_id) {
          this.ducPrice = (result.token_type * result.gold_price * result.duc_value) / result.duc_count;
          this.winData = result;
